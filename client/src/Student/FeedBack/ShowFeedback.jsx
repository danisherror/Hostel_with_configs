...
if (confirmation) {
    ...
    const res = await fetch(`http://localhost:8000/api/v1/deletefeedback/${id}`, {
        method: "DELETE",
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="scroll-container" style={{ maxHeight: '500px', overflowY: 'scroll' }}>
          <table className="w-full table-auto">
            ...
            <tbody>
              {feedback.map((feedbackEntry, key) => (
                <tr key={key}>
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    ...
                    <button className="flex justify-center rounded bg-primary py-2 px-6 font-medium text-gray hover:bg-opacity-90"
                      onClick={() => deleteFeedback(feedbackEntry._id)} >
                      Delete</button>
                  </td>
                </tr>
              ))}

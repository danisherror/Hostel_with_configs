...
const SignUp: React.FC = () => {
  ...
  const handleUserTypeChange = (event) => {
    setFormData((prevData) => ({ ...prevData, hostelName: event.target.value }));
  };
  const getToken = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/v1/wsignup`, {
        ...
        body: JSON.stringify({
          name, email, password, url1, collegeid, phone, hostelName
        })
      });

      if (response.status === 200) { // Assuming 201 (Created) for successful signup
        alert(`Registered successfully warden!`);
        ...
        alert("Warden already exist for this hostel")
      }
      else {
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Create Warden" />
